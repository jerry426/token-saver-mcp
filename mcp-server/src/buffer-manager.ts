...
const TOOL_DEPTH_LIMITS: { [toolName: string]: number } = {
  find_text: 2, // Shallow for search results
  find_symbols: 3, // Medium for symbol results
  ...
  get_diagnostics: 3, // Medium depth
  get_semantic_tokens: 2, // Shallow for token arrays
}
...
function generateSmartPreview(
  ...
  if ((toolName === 'find_text' || toolName === 'find_symbols') && Array.isArray(data)) {
  ...
  if ((toolName === 'get_references' || toolName === 'find_implementations') && Array.isArray(data)) {
    const byFile: { [key: string]: any[] } = {}

    for (const item of data) {
      const uri = item.uri?.toString?.() || item.location?.uri?.toString?.() || 'unknown'
  ...
  if (toolName === 'get_completions') {
    ...
    if (!Array.isArray(items)) {
      return data // Can't process non-array data
    }
...
export function bufferResponse(
  ...
  logger.info(`[BUFFER_MANAGER] ${toolName} response analysis:`, {
    tokens,
    bytes,
    depth,
    itemCount,
  ...
  switch (toolName) {
    ...
    case 'get_document_symbols':
      if (metadata.maxDepth > 4) {
        ...
        suggestions.push('Use find_symbols for specific symbol names')
      }
      break
    ...
    case 'get_diagnostics':
      if (metadata.itemCount > 100) {
        suggestions.push('Many diagnostics found, consider filtering by severity')
        suggestions.push('Focus on specific files or directories')
      }
      break
  }
